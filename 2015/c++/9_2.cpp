...
void add_city_info(const std::string &str) {
  ...
  auto cities_info = Text::split(point_to_point, " to ");
  auto from_city = std::string(cities_info[0]);
  auto to_city = std::string(cities_info[1]);
...
int calc_route_len(const std::vector<std::string> route) {
  auto total = 0;
  for (uint64_t idx = 0; idx < route.size() - 1; ++idx) {
